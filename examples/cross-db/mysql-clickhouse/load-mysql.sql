-- MySQL seed data for mysql-clickhouse comparison test
-- Target row count: 10000 rows in each source table
-- This is the same base data as mysql-pg; ClickHouse target has intentional inconsistencies
...
SELECT
    ...
    CASE MOD(n, 4) WHEN 0 THEN 'new' WHEN 1 THEN 'processing' WHEN 2 THEN 'done' ELSE 'failed' END AS col_enum,
    CASE MOD(n, 3) WHEN 0 THEN 'a,b' WHEN 1 THEN 'b' ELSE 'c' END AS col_set,
    ...
    CASE MOD(n, 4) WHEN 0 THEN 'Shanghai' WHEN 1 THEN 'Beijing' WHEN 2 THEN 'Shenzhen' ELSE 'Hangzhou' END AS city,
    'CN' AS country,
    ...
    CASE MOD(n, 4) WHEN 0 THEN 'active' WHEN 1 THEN 'inactive' WHEN 2 THEN 'pending' ELSE 'blocked' END AS status,
    CASE MOD(n, 5) WHEN 0 THEN 'retail' WHEN 1 THEN 'finance' WHEN 2 THEN 'logistics' WHEN 3 THEN 'manufacturing' ELSE 'public' END AS category,
...
INSERT INTO mydb.users (id, name, email, phone, status, created_at)
SELECT
    n AS id,
    CONCAT('user_', LPAD(n, 5, '0')) AS name,
    CONCAT('user_', LPAD(n, 5, '0'), '@example.com') AS email,
    CONCAT('+861380', LPAD(n, 6, '0')) AS phone,
    CASE MOD(n, 3) WHEN 0 THEN 'active' WHEN 1 THEN 'inactive' ELSE 'pending' END AS status,
