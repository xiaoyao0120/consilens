-- TiDB seed data for mysql-tidb comparison test
-- TiDB is MySQL-compatible, so SQL is similar to MySQL but with intentional inconsistencies
-- Purpose: MySQL source has "correct" data; TiDB target has known differences
...
-- =========================================================
-- consilens_performance_demo_table (TiDB target)
-- =========================================================
...
SELECT 'tidb.mydb.users' AS check_name, COUNT(*) AS actual_rows, 10000 AS expected_rows,
    MIN(id) AS min_id, MAX(id) AS max_id FROM mydb.users;

SELECT 'tidb.mydb.orders' AS check_name, COUNT(*) AS actual_rows, 10000 AS expected_rows,
    ROUND(SUM(amount),4) AS amount_sum FROM mydb.orders;

SELECT 'tidb.production.fact_orders' AS check_name, COUNT(*) AS actual_rows, 10000 AS expected_rows,
