-- Trino target seed data for mysql-trino comparison test
-- Trino connects to MySQL via its MySQL connector (jdbc:trino://host:port/mysql/schema)
-- The "target" data lives in a separate MySQL database (consilens_demo_target) with intentional inconsistencies
-- Trino queries this database through its MySQL connector

SET NAMES utf8mb4;
SET time_zone = '+08:00';

-- Create a separate target database for Trino to query
...
-- =========================================================
-- consilens_performance_demo_table (Trino target via MySQL connector)
-- =========================================================
...
SELECT 'trino_target.consilens_demo_target.consilens_performance_demo_table' AS check_name,
...
SELECT 'trino_target.mydb_target.users' AS check_name, COUNT(*) AS actual_rows, 10000 AS expected_rows,
    MIN(id) AS min_id, MAX(id) AS max_id FROM mydb_target.users;

SELECT 'trino_target.mydb_target.orders' AS check_name, COUNT(*) AS actual_rows, 10000 AS expected_rows,
    ROUND(SUM(amount),4) AS amount_sum FROM mydb_target.orders;

SELECT 'trino_target.production_target.fact_orders' AS check_name, COUNT(*) AS actual_rows, 10000 AS expected_rows,
    ROUND(SUM(total_amount),4) AS total_amount_sum FROM production_target.fact_orders;

SELECT 'trino_target.consilens_demo_target.daily_order_summary' AS check_name,
