-- Presto target seed data for mysql-presto comparison test
-- Presto connects to MySQL via its MySQL connector (jdbc:presto://host:port/mysql/schema)
-- The "target" data lives in a separate MySQL database (consilens_demo_target) with intentional inconsistencies
-- Presto queries this database through its MySQL connector

SET NAMES utf8mb4;
SET time_zone = '+08:00';

-- Create a separate target database for Presto to query
CREATE DATABASE IF NOT EXISTS consilens_demo_target
    DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;

CREATE DATABASE IF NOT EXISTS mydb_target
    DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;

CREATE DATABASE IF NOT EXISTS production_target
    DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;

USE consilens_demo_target;
...
-- =========================================================
-- consilens_performance_demo_table (Presto target via MySQL connector)
-- =========================================================

DROP TABLE IF EXISTS consilens_demo_target.consilens_performance_demo_table;

CREATE TABLE consilens_demo_target.consilens_performance_demo_table (
...
INSERT INTO consilens_demo_target.consilens_performance_demo_table
SELECT
...
UPDATE consilens_demo_target.consilens_performance_demo_table SET amount = 99999.9999 WHERE record_id = 'REC0000000001';
UPDATE consilens_demo_target.consilens_performance_demo_table SET status = 'modified_status' WHERE record_id = 'REC0000000002';
...
DROP TABLE IF EXISTS mydb_target.users;
CREATE TABLE mydb_target.users (
...
UPDATE mydb_target.users SET email = 'modified@example.com' WHERE id = 1;

-- =========================================================
-- mydb_target.orders
-- =========================================================

DROP TABLE IF EXISTS mydb_target.orders_backup;
DROP TABLE IF EXISTS mydb_target.orders;
CREATE TABLE mydb_target.orders (
...
INSERT INTO mydb_target.orders
...
CREATE TABLE mydb_target.orders_backup LIKE mydb_target.orders;
INSERT INTO mydb_target.orders_backup SELECT * FROM mydb_target.orders;
...
-- =========================================================
-- production_target.fact_orders
-- =========================================================

DROP TABLE IF EXISTS production_target.fact_orders;
CREATE TABLE production_target.fact_orders (
...
INSERT INTO production_target.fact_orders
...
DROP TABLE IF EXISTS consilens_demo_target.daily_order_summary;
CREATE TABLE consilens_demo_target.daily_order_summary (
...
INSERT INTO consilens_demo_target.daily_order_summary
SELECT
    COALESCE(DATE(created_at), '2026-05-01') AS biz_date,
    status,
    COUNT(*) AS order_count,
    CAST(SUM(amount) AS DECIMAL(38,4)) AS total_amount,
    MAX(updated_at) AS updated_at
FROM consilens_demo_target.consilens_performance_demo_table
WHERE deleted = 0
GROUP BY COALESCE(DATE(created_at), '2026-05-01'), status;
...
SELECT 'presto_target.consilens_demo_target.consilens_performance_demo_table' AS check_name,
...
SELECT 'presto_target.mydb_target.users' AS check_name, COUNT(*) AS actual_rows, 10000 AS expected_rows,
    MIN(id) AS min_id, MAX(id) AS max_id FROM mydb_target.users;

SELECT 'presto_target.mydb_target.orders' AS check_name, COUNT(*) AS actual_rows, 10000 AS expected_rows,
    ROUND(SUM(amount),4) AS amount_sum FROM mydb_target.orders;

SELECT 'presto_target.production_target.fact_orders' AS check_name, COUNT(*) AS actual_rows, 10000 AS expected_rows,
    ROUND(SUM(total_amount),4) AS total_amount_sum FROM production_target.fact_orders;

SELECT 'presto_target.consilens_demo_target.daily_order_summary' AS check_name,
    COUNT(*) AS actual_groups,
    ROUND(SUM(total_amount),4) AS total_amount_sum
FROM consilens_demo_target.daily_order_summary;
