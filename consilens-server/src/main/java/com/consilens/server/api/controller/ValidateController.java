...
public class ValidateController {
    ...
    public ResponseEntity<ApiResponse<SyncArtifactResponse>> validate(@Valid @RequestBody ValidateRequest request,
                                                                      HttpServletRequest httpServletRequest) {
        String traceId = TraceIdSupport.getOrCreateTraceId(httpServletRequest);
        return ResponseEntity.ok(ApiResponse.success(synchronousCapabilityService.validate(request, traceId), traceId));
    }
}
