...
/**
 * Unit tests for OceanBaseSqlQueryGenerator.
 */
class OceanBaseSqlQueryGeneratorTest {

    private OceanBaseSqlQueryGenerator generator;
    private OceanBaseCapabilityProvider capabilityProvider;
    private OceanBaseDataTypeHandler dataTypeHandler;

    @BeforeEach
    void setUp() {
        capabilityProvider = new OceanBaseCapabilityProvider();
        dataTypeHandler = new OceanBaseDataTypeHandler(capabilityProvider);
        generator = new OceanBaseSqlQueryGenerator(capabilityProvider, dataTypeHandler);
    }
    ...
    void testGetFullOuterJoinSQL() {
        ...
        assertFalse(sql.contains("UNION"), "OceanBase should use native FULL OUTER JOIN, not UNION simulation");
    }

    @Test
    void testGetFullOuterJoinSQLWithWhere() {
        String sql = generator.getFullOuterJoinSQL("table1", "table2",
                Arrays.asList("id"), "table1.status = 'active'");

        assertTrue(sql.contains("FULL OUTER JOIN"));
        assertTrue(sql.contains("WHERE table1.status = 'active'"));
    }
    ...
    @Test
    void testGetRowHashSQLWithWhereClause() {
        Map<String, DataType> types = new HashMap<>();
        types.put("id", DataType.INTEGER);
        types.put("name", DataType.VARCHAR);
    ...
    @Test
    void testGetRowHashSQLWithCompositePrimaryKey() {
        Map<String, DataType> types = new HashMap<>();
    ...
    @Test
    void testGetRowHashSQLWithoutSchema() {
        Map<String, DataType> types = new HashMap<>();
        types.put("id", DataType.INTEGER);
        types.put("name", DataType.VARCHAR);
    ...
    @Test
    void testGetRowHashSQLWithVariousDataTypes() {
        Map<String, DataType> types = new HashMap<>();
        types.put("id", DataType.INTEGER);
        types.put("name", DataType.VARCHAR);
        types.put("amount", DataType.DECIMAL);
    ...
    void testGetRowHashSQLOutput() {
        ...
        System.out.println("\n=== Generated Row-Hash SQL (OceanBase) ===");
