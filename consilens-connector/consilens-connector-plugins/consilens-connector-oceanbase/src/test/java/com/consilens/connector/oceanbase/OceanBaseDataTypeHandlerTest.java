...
/**
 * Unit tests for OceanBaseDataTypeHandler.
 */
class OceanBaseDataTypeHandlerTest {

    private OceanBaseDataTypeHandler handler;
    private OceanBaseCapabilityProvider capabilityProvider;

    @BeforeEach
    void setUp() {
        capabilityProvider = new OceanBaseCapabilityProvider();
        handler = new OceanBaseDataTypeHandler(capabilityProvider);
    }
    ...
    void testNormalizeColumn_TimestampDateOnlyMode() {
        ...
        handler = new OceanBaseDataTypeHandler(capabilityProvider, Map.of("timestamp", rule));
    ...
    void testNormalizeColumn_TimestampWithTimezoneUsesExactRuleBeforeFallback() {
        ...
        handler = new OceanBaseDataTypeHandler(capabilityProvider, Map.of(
    ...
    void testNormalizeColumn_TimeWithTimezoneUsesExactRule() {
        JdbcDatasetHandle.JdbcTypeNormalizationRule rule = new JdbcDatasetHandle.JdbcTypeNormalizationRule();
        rule.setFormat("%H:%i");
        handler = new OceanBaseDataTypeHandler(capabilityProvider, Map.of("time_with_timezone", rule));
    ...
    @Test
    void shouldConvertLargeintDescriptor() {
        // OceanBase specific: LARGEINT is 128-bit integer
        TypeDescriptor descriptor = handler.convertToTypeDescriptor("largeint");

        assertEquals(com.consilens.common.enums.DataType.INTEGER_TYPE, descriptor.getType());
        assertEquals(128, descriptor.getBitWidth());
    }

    @Test
    void shouldConvertLargeintUnsignedDescriptor() {
        TypeDescriptor descriptor = handler.convertToTypeDescriptor("largeint unsigned");

        assertEquals(com.consilens.common.enums.DataType.INTEGER_TYPE, descriptor.getType());
        assertEquals(128, descriptor.getBitWidth());
        assertTrue(descriptor.isUnsigned());
        // convertToOriginType returns the original type string as-is
        assertEquals("largeint unsigned", handler.convertToOriginType(descriptor));
    }
}
